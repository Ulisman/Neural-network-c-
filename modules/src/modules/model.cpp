...
void Model::backprop(Eigen::MatrixXf& modelOutput, Eigen::MatrixXf& predictionError, float& lr){
    //start with deltas and gradients for output layer
    Eigen::MatrixXf outputDeltas = predictionError.cwiseProduct(modelOutput.unaryExpr(std::function<float(float)>(activations::sigmoidDerivative))); //Hadamard product
